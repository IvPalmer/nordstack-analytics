...
make. If port 5432 or 3306 is taken, copy `.env.example` to `.env` and change the ports.
...
## Layout
    ...
    tests/sources/        singular diagnostics (warn)
...
cells are NULL, as the source database would store them.
...
precedes its start date (its invoices are real; its end date is nulled, so it is neither
churned nor cancelled at the cutoff).
...
| 9 | End date before start date, 17 invoices after it | S00034 | singular tests | end_date NULL; invoices kept; not churned |
...
tests tie paid revenue, LTV and churn back to the source; coverage tests prove the
monthly marts have exactly one row per spine month. Everything after staging is ERROR.
